...
function CompanyPostsComp({companies}: {companies: CompanyData[]}) {
    ...
    const totalPages = Math.ceil(companies.length / jobsPerPage)
                      ...
                      <div className="p-6  rounded-md border border-[#EDEDED]">
                          <div className="flex items-start justify-between gap-4">
                                ...
                                <div className="flex flex-col gap-2.5 w-full leading-none">
                                    ...
                                    <div className="flex items-center gap-0.5 text-[12px] text-[#757575+3]">
                                      <MapPin size={10} />
                                      <span>{company.addresses.find(add => add.isHeadQuarters)?.city ?? company.addresses[0]?.city ?? "Remote"}</span>
                                    </div>
                                    <div className="flex items-center gap-1 ">
                                        <Badge variant="outline" className="text-joblin-primary bg-[#E6F8F1]" >{company.domain}</Badge>
                                    </div>
                                    <p className='text-[14px] md:text-[16px] text-joblin-black'>{company.description}</p>
                                    <div className="flex items-center justify-between font-medium ">
                                      <p className="text-joblin-light-gray text-[12px] lg:text-[14px]">{company.companySize} Employee</p>
                                        <div className=" text-joblin-primary flex  md:items-center text-[14px] gap-2 cursor-pointer font-medium"> 
                                        <Link href={`/companies/${company.id}`} className=" text-joblin-dark-gray hover:text-joblin-primary cursor-pointer ">
...
export default CompanyPostsComp
